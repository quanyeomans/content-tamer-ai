...
class ErrorClassifier:
    ...
    def classify_error(exception: Exception) -> ErrorClassification:
        ...
        if error_code == errno.EACCES or "permission denied" in error_str or "errno 13" in error_str:
        ...
        # File in use errors
        if error_code == errno.EBUSY or "file is being used" in error_str or "errno 32" in error_str:
            return ErrorClassification(
        ...
        # File format issues
        format_indicators = ["unsupported", "invalid format", "corrupted", "not a valid"]
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                if attempt > 0:
                    ...
                    display_context.show_info(f"✅ Successfully processed after retry")
                else:
                ...
                if (error_classification.is_recoverable and 
                    error_classification.retry_recommended and 

...
class MessageHandler:
    ...
    def info(
        ...
        if self.config.show_info and not self.config.quiet_mode and \
           self._should_display_message(MessageLevel.INFO, priority):
    ...
    def success(
        ...
        if self.config.show_success and not self.config.quiet_mode and \
           self._should_display_message(MessageLevel.SUCCESS, priority):
            self._display_message_enhanced(message, MessageLevel.SUCCESS, location, priority, context)
    ...
    def warning(
        ...
        if self.config.show_warnings and \
           self._should_display_message(MessageLevel.WARNING, priority):
            self._display_message_enhanced(message, MessageLevel.WARNING, location, priority, context)
    ...
    def error(
        ...
        if self.config.show_errors and \
           self._should_display_message(MessageLevel.ERROR, priority):
    ...
    def critical(
        ...
        self._display_message_enhanced(message, MessageLevel.CRITICAL, location, MessagePriority.CRITICAL, context)
    ...
    def _format_enhanced_message(
        ...
        elif priority == MessagePriority.HIGH and self.config.disclosure_mode == DisclosureMode.DEBUG:
    ...
    def show_summary(self) -> None:
        ...
        if self.session_stats['warning'] > 0:
            summary_parts.append(
                self.formatter.colorize(f"{self.session_stats['warning']} warnings", "bright_yellow")
            )
        ...
        if self.session_stats['critical'] > 0:
            summary_parts.append(
                self.formatter.colorize(f"{self.session_stats['critical']} critical errors", "bright_red", bold=True)
            )
    ...
    def _show_detailed_stats(self) -> None:
        ...
        if self.message_counts:
            ...
            sorted_patterns = sorted(self.message_counts.items(), key=lambda x: x[1], reverse=True)[:3]
            for pattern, count in sorted_patterns:
                if count > 1:
                    pattern_display = self.formatter.colorize(f"  \"{pattern}...\": {count} times", "dim")

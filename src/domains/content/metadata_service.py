...
class MetadataExtractor:
    ...
    def extract_comprehensive_metadata(self, file_path: str, content: ExtractedContent) -> DocumentMetadata:
...
class MetadataAnalyzer:  # Renamed to avoid duplicate class names
    ...
    def analyze_document_type(self, metadata: DocumentMetadata) -> Dict[str, Any]:
        ...
        if metadata.has_numbers:
            financial_score += 0.2
        if any(word in metadata.file_name.lower() for word in ['invoice', 'bill', 'statement', 'receipt']):
            financial_score += 0.4
    ...
    def get_document_summary(self, metadata: DocumentMetadata) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_important_data": any([
                metadata.has_dates,
                metadata.has_currency,
                metadata.has_numbers
            ])
        }
...
class MetadataService:
    ...
    def batch_analyze(self, file_paths: List[str], content_map: Dict[str, ExtractedContent]) -> Dict[str, DocumentMetadata]:

...
def _extract_file_content(input_path: str, ocr_lang: str, display_context: Any) -> Tuple[str, str]:
...
def _generate_filename(
    ...
    try:
        ...
        # Final fallback if AI generation completely fails
        if display_context:
            display_context.show_error(f"AI filename generation failed: {str(e)}")
        timestamp = dt.datetime.now().strftime("%Y%m%d%H%M%S")
        fallback_name = f"ai_failed_{timestamp}"
        validated_filename = organizer.filename_handler.validate_and_trim_filename(fallback_name)
...
def process_file_enhanced_core(
    ...
    try:
        ...
        # Catch-all for any unexpected exceptions (including AI failures)
        try:
            if os.path.exists(input_path):
                unprocessed_path = os.path.join(unprocessed_folder, filename)
                organizer.file_manager.safe_move(input_path, unprocessed_path)
                display_context.show_error(
                    f"Unexpected error, file moved to unprocessed: {str(e)}",
                    filename=filename,
                )
            else:
                display_context.show_error(f"Unexpected error: {str(e)}", filename=filename)
        except Exception as move_error:
            display_context.show_error(f"Critical error: {str(move_error)}", filename=filename)
...
def get_new_filename_with_retry_enhanced(
    ...
    for attempt in range(max_attempts):
        ...
        except Exception as e:  # Catch all AI provider exceptions, not just RuntimeError
...
def get_new_filename_with_retry(
    ...
    image_b64: Optional[str] = None,
    max_attempts: int = MAX_RETRIES,
    max_retries: Optional[int] = None,  # Backward compatibility
) -> str:
    """

...
try:
    ...
    SafeFileManager = None  # type: ignore
    FileLockManager = None  # type: ignore
    _UNIFIED_FILE_MANAGER_AVAILABLE = False


class FileManager:
    """Cross-platform file operations with locking and retry logic.

    LEGACY COMPATIBILITY: This class is deprecated in favor of SharedFileManager.
    Maintained for backwards compatibility during architecture migration.
    """
    ...
    def lock_file(self, file_obj: TextIO) -> None:
        """Acquire exclusive file lock (cross-platform)."""
        if self._lock_manager:
            # Use new unified lock manager
            self._lock_manager.lock_file(file_obj)
        else:
            ...
            if platform.system() == "Windows":
                ...
                if fcntl is not None:
                    fcntl.flock(file_obj.fileno(), fcntl.LOCK_EX)  # type: ignore[attr-defined]

    def unlock_file(self, file_obj: TextIO) -> None:
        """Release file lock (cross-platform)."""
        if self._lock_manager:
            # Use new unified lock manager
            self._lock_manager.unlock_file(file_obj)
        else:
            # Legacy implementation
            if platform.system() == "Windows":
                if msvcrt is not None:
    ...
    def safe_move(self, src: str, dst: str, attempts: int = 3, delay: float = 0.75) -> None:
        """Robust file move with retry logic and copy-delete fallback."""
        if self._unified_manager:
            # Use new unified file manager
            success = self._unified_manager.safe_move(src, dst, attempts, delay)
            if not success:
                raise OSError(f"Failed to move {src} to {dst}")
        else:
            # Legacy implementation
            last_err = None
    ...
    @staticmethod
    def get_unified_manager() -> Any:  # type: ignore
        """Get unified file manager if available."""
        if _UNIFIED_FILE_MANAGER_AVAILABLE and SafeFileManager is not None:
    ...
    def run_post_processing_organization(
        ...
        try:
            ...
            clustering_config = ClusteringConfig(
                ml_threshold=0.7 if ml_enhancement_level >= 2 else 0.9,

...
class PDFAnalyzer:
    ...
    def _check_pdfid_availability(self) -> bool:
        ...
        try:
            from pdfid.pdfid import PDFiD  # pylint: disable=unused-import
    ...
    def _run_pdfid_analysis(self, file_path: str) -> PDFThreatAnalysis:
        ...
        try:
            ...
            raise Exception("PDFiD encountered a fatal error during analysis") from exc
        except Exception as e:
            raise Exception(f"PDFiD analysis failed: {str(e)}") from e
    ...
    def _extract_indicators(self, xmldoc) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(xmldoc, "getroot"):
                root_obj = xmldoc.getroot()
                root = root_obj() if callable(root_obj) else root_obj
            else:
                # Handle string output - use safe parsing with defusedxml
                root = ET.fromstring(str(xmldoc))  # nosec B314 - using defusedxml

            # Extract key threat indicators
            if not hasattr(root, "findall"):
                raise Exception("Invalid XML root element - missing findall method")

            # Type assertion for Pyright - we've verified it has findall method
            xml_root = cast(Any, root)
            for keyword in xml_root.findall(".//Keyword"):
...
def sanitize_log_message(message: str) -> str:
    ...
    patterns = [
        ...
        (r"sk-ant-[A-Za-z0-9_-]{15,}", lambda m: "sk-ant-***"),
        # Google API keys: AIza... (typically 39+ chars total, so 35+ after prefix)
        (r"AIza[A-Za-z0-9_-]{30,}", lambda m: "AIza***"),
        # Also catch shorter test keys for development/testing
        (r"sk-proj-test[A-Za-z0-9_-]+", lambda m: "sk-proj-test***"),
        (r"sk-ant-test[A-Za-z0-9_-]+", lambda m: "sk-ant-***"),
    ]
...
def sanitize_environment_vars(env_vars: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    secret_patterns = ["API_KEY", "SECRET", "TOKEN", "PASSWORD", "PRIVATE_KEY", "AUTH"]

...
def _display_api_key_instructions(provider: str) -> None:
    """Display instructions for API key input."""
    print("\nAPI Key Input:")
...
def _clear_screen_secure():
    """
    Securely clear the terminal screen without shell injection risks.

    Uses ANSI escape sequences when possible, subprocess as fallback.
    """
    try:

    ...
    if shutil.which(tesseract_cmd):
        try:
            result = subprocess.run(
                [tesseract_cmd, "--version"], capture_output=True, text=True, check=True
            )
...
def get_ai_provider_options():
    ...
    return {
        "openai": {
            ...
            "description": "OpenAI GPT models (GPT-5, GPT-4o with vision)",
            "status": "[OK] Production Ready",
        },
        "claude": {
            "package": "anthropic>=0.34.0",
            "description": "Anthropic Claude models (Opus 4.1, Sonnet 4, Claude 3.5 series)",
            "status": "[OK] Latest Models",
        },
        "gemini": {
            "package": "google-genai>=0.7.0",
...
def prompt_user_consent(packages, package_type="dependencies"):
    ...
    while True:
        response = (
            input(f"Do you consent to install these packages? {Colors.BOLD}[y/N]{Colors.END}: ")
            .strip()
            .lower()
        )
...
def select_ai_providers():
    ...
    for i, (key, info) in enumerate(providers.items(), 1):
        status_color = (
            ...
            else Colors.YELLOW if "[!]" in info["status"] else Colors.CYAN
        )
        print(f"  {i}. {Colors.BOLD}{key.title()}{Colors.END} - {info['description']}")
        print(f"     {status_color}{info['status']}{Colors.END}")
        print()

    print("  0. Skip AI provider installation (install manually later)")
    print()
...
def create_virtual_environment():
    ...
    while True:
        response = (
            input(f"Create virtual environment? {Colors.BOLD}[Y/n]{Colors.END}: ").strip().lower()
        )

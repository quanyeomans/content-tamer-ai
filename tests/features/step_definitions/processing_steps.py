...
def lock_file_temporarily(bdd_context):
    ...
    bdd_context.locked_file = bdd_context.test_files[1] if len(bdd_context.test_files) > 1 else bdd_context.test_files[0]
...
def verify_successful_files_count(bdd_context, num):
    ...
    assert len(processed_files) == num, f"Expected {num} processed files, found {len(processed_files)}: {processed_files}"
...
def verify_progress_completion(bdd_context):
    ...
    assert "100%" in display_text or "completed" in display_text.lower(), \
...
def verify_retry_success(bdd_context, num):
    ...
    assert len(processed_files) == total_files, \
...
def verify_recovery_statistics(bdd_context):
    ...
    assert any(word in display_text.lower() for word in ["retry", "recovered", "attempt"]), \
...
@then(parsers.parse('I should see accurate final statistics showing {success:d} success, {failure:d} failure'))

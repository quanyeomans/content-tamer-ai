...
# Scenario imports
@scenario('../platform_consistency.feature', 'Processing works consistently with different path separators')
...
@scenario('../platform_consistency.feature', 'Display output works across different terminal environments')
...
def clean_platform_test_environment(platform_context):
    ...
    platform_context.nested_dir = os.path.join(platform_context.input_dir, "nested", "deeply", "nested")
...
def verify_platform_appropriate_errors(platform_context):
    ...
    has_expected_error = any("expected_error" in scenario for scenario in platform_context.error_scenarios)
    has_unexpected_crash = any("unexpected_error" in scenario for scenario in platform_context.error_scenarios)
    
    # Either we got expected errors OR the platform allows the operation
    assert has_expected_error or not has_unexpected_crash, \
...
def verify_display_works(platform_context):
    ...
    assert quiet_test['length'] <= normal_test['length'], \
...
def verify_special_character_handling(platform_context):
    ...
    try:
        ...
        assert replacement_char_count < len(decoded) * 0.1, \
            f"Too many encoding issues in display output. Replacement chars: {replacement_char_count}"
            
    except Exception as e:
        pytest.fail(f"Display output should handle encoding gracefully: {e}")
...
def simulate_retry_success(error_context):
    ...
    def mock_operation():
        ...
        return "successfully_processed_file.pd"
    ...
    with error_context.display_manager.processing_context(total_files=1) as display_context:
        ...
        success, result, error_classification = retry_handler.execute_with_retry(
            ...
            filename="test_file.pd",
        )

        # Complete the file based on retry result
        if success:
            display_context.complete_file("test_file.pd", result)
        else:
            display_context.fail_file(
                "test_file.pd", f"Failed after retries: {error_classification}"
            )

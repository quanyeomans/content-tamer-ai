...
@scenario('../error_handling.feature', 'User sees recovery information when files succeed after retries')
def test_user_sees_recovery_information_when_files_succeed_after_retries():
    pass


@scenario('../error_handling.feature', 'User sees accurate progress statistics during mixed outcomes')
...
def simulate_retry_success(error_context):
    ...
    with error_context.display_manager.processing_context(total_files=1) as display_context:
        ...
        if success:
            ...
            display_context.fail_file("test_file.pdf", f"Failed after retries: {error_classification}")
...
def verify_clear_error_communication(error_context):
    ...
    assert "error" in display_output.lower() or "failed" in display_output.lower(), \
...
def verify_error_count_incremented(error_context):
    ...
    assert final_error_count == expected_error_count, \
...
def verify_final_result_shows_success(error_context):
    ...
    assert final_success_count == error_context.initial_success_count + 1, \
        "Retry success should increment success counter"
    assert final_error_count == error_context.initial_error_count, \
...
def verify_retry_statistics(error_context):
    ...
    assert error_context.retry_result == "successfully_processed_file.pdf", \
...
def verify_statistics_consistency(error_context):
    ...
    assert test_success_count + test_error_count == 5, \

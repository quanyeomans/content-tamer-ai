...
def step_rich_ui_enabled(context):
    ...
    context.console = Console(
        file=context.output_buffer, force_terminal=True, width=120, no_color=False
    )
...
def step_complete_processing_file(context, filename, target_filename):
    ...
    context.processed_files.append(
        {"source": filename, "target": target_filename, "status": "success"}
    )
...
def step_complete_processing_successfully(context, filename):
    ...
    target_filename = "Processed_{filename}"
...
def step_fail_processing_file(context, filename, error_message):
    ...
    context.failed_files.append({"source": filename, "error": error_message, "status": "failed"})
    context.files_completed += 1
...
def step_complete_both_documents(context):
    ...
    for i in range(2):
        filename = "test_doc_{i+1}.pdf"
        target_filename = "Processed_Doc_{i+1}.pdf"
        context.progress_display.complete_file(filename, target_filename)
...
def step_complete_files_successfully(context, success_count):
    ...
    for i in range(success_count):
        filename = "success_{i+1}.pdf"
        target_filename = "Success_Processed_{i+1}.pdf"
        step_complete_processing_file(context, filename, target_filename)
...
def step_fail_files_with_errors(context, fail_count):
    ...
    for i in range(fail_count):
        filename = "failed_{i+1}.pdf"
        error_message = "Processing error {i+1}"
...
def step_complete_all_files(context, file_count):
    ...
    for i in range(file_count):
        filename = "batch_file_{i+1}.pdf"
        target_filename = "Batch_Processed_{i+1}.pdf"
        step_complete_processing_file(context, filename, target_filename)
...
def step_processing_encounters_errors(context):
    ...
    step_complete_processing_file(context, "success1.pd", "Success1_Processed.pd")
    step_fail_processing_file(context, "error1.pd", "Network timeout")
    step_complete_processing_file(context, "success2.pd", "Success2_Processed.pd")
    step_fail_processing_file(context, "error2.pd", "File corrupted")
...
def step_intensive_processing(context):
    ...
    for i in range(10):
        filename = "intensive_doc_{i+1}.pdf"
        context.progress_display.start_file(filename)
        time.sleep(0.05)  # Simulate processing time
        context.progress_display.complete_file(filename, "Intensive_Processed_{i+1}.pdf")
...
def step_see_file_in_display(context, file_display):
    ...
    if " → " in file_display:
        ...
        assert target_file.strip() in output, "Target file {target_file} should be in display"
    else:
        assert file_display in output, "File {file_display} should be in display"
...
def step_see_file_displayed_below(context, file_display):
    ...
    assert completed_count >= 2, "Should have at least 2 completed files, got {completed_count}"
...
def step_see_all_files_persistent(context):
    ...
    assert len(completed_files) == 3, "Should have 3 completed files, got {len(completed_files)}"

    output = context.output_buffer.getvalue()
    for file_info in completed_files:
        assert file_info["filename"] in output, "File {file_info['filename']} should be in output"
...
def step_display_completion_stats(context):
    ...
    assert stats.succeeded == 3, "Should show 3 successes, got {stats.succeeded}"
    assert stats.completed == 3, "Should show 3 completed, got {stats.completed}"
...
def step_no_border_fragments(context):
    ...
    for border_line in border_lines:
        ...
        if border_content:  # If it has border chars
            assert len(border_content) > 3, "Border line too short (fragment): {border_line}"
...
def step_see_success_icon(context, filename):
    ...
    output = context.output_buffer.getvalue()
    assert filename in output, "Filename {filename} should be in output"
    ...
    assert success_file is not None, "Should find {filename} in completed files"
    assert success_file["status"] == "success", "File {filename} should have success status"
    assert success_file["icon"] == "✅", "File {filename} should have success icon"
...
def step_see_error_icon(context, filename, error_text):
    ...
    assert error_file is not None, "Should find {filename} in completed files"
    assert error_file["status"] == "failed", "File {filename} should have failed status"
    assert error_file["icon"] == "❌", "File {filename} should have error icon"
    assert error_file.get("error") == error_text, "Should have error message: {error_text}"
...
def step_all_files_visible_persistent(context):
    ...
    completed_files = context.progress_display._completed_files
    assert len(completed_files) == 3, "Should have 3 files in persistent display"
...
def step_statistics_show_text(context, expected_text):
    ...
    if "successful files" in expected_text:
        ...
        assert stats.succeeded == expected_count, "Should have {expected_count} successes"

    elif "failed files" in expected_text:
        expected_count = int(expected_text.split()[0])
        assert stats.failed == expected_count, "Should have {expected_count} failures"
        ...
        ), "Success rate should be {expected_rate}%, got {actual_rate}%"
...
def step_display_shows_last_files(context, count):
    ...
    assert len(completed_files) == 8, "Should track all 8 files"
...
def step_progress_100_percent(context):
    ...
    assert completion_rate == 100.0, "Should show 100% completion, got {completion_rate}%"
...
def step_professional_engaging_display(context):
    ...
    ), "Should have multiple Rich UI elements, found: {found_elements}"
...
def step_rich_ui_smooth_updates(context):
    ...
    if hasattr(context, "performance_start"):
        ...
        assert elapsed < 2.0, "Processing should complete quickly, took {elapsed}s"

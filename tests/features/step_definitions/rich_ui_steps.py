...
def step_statistics_show_text(context, expected_text):
    ...
    if "successful files" in expected_text:
        ...
        assert abs(actual_rate - expected_rate) < 0.1, "Success rate should be {expected_rate}%, got {actual_rate}%"
...
def step_professional_engaging_display(context):
    ...
    assert len(found_elements) >= 3, "Should have multiple Rich UI elements, found: {found_elements}"

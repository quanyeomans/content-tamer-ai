"""
Tests for Unified Display Manager in Shared Display Layer.
"""
...
class TestUnifiedDisplayManager(unittest.TestCase, RichTestCase):
    ...
    def setUp(self):
        """Set up test fixtures with Rich console isolation."""
        RichTestCase.setUp(self)
        # Create display manager with test console
        self.display_manager = UnifiedDisplayManager(console=self.test_console, quiet_mode=False)

    def test_display_manager_initialization(self):
        """Test display manager initializes correctly."""
        self.assertIsInstance(self.display_manager, UnifiedDisplayManager)
        self.assertIsNotNone(self.display_manager.console)
        self.assertFalse(self.display_manager.quiet_mode)

    def test_info_message_display(self):
        """Test info message display through Rich Console."""
        self.display_manager.info("Test info message")

        output = self.get_console_output()
        self.assertIn("Test info message", output)
        # Rich styling should be preserved in test output

    def test_success_message_display(self):
        """Test success message display with Rich styling."""
        self.display_manager.success("Test success message")
    ...
    def test_error_message_display(self):
        ...
        self.display_manager.error("Test error", suggestions=["Fix 1", "Fix 2"])
    ...
    def test_quiet_mode_suppression(self):
        ...
        quiet_display = UnifiedDisplayManager(console=self.test_console, quiet_mode=True)

        quiet_display.info("Should be suppressed")
...
class TestProcessingContext(unittest.TestCase):
    ...
    def test_start_file(self):
        ...
        self.context.start_file("test.pd")
        self.assertEqual(self.context.current_file, "test.pd")
...
class TestDisplayManager(unittest.TestCase):
    ...
    def test_processing_context_creation(self):
        ...
        with self.display_manager.processing_context(total_files=3, description="Testing") as ctx:
            ...
            ctx.complete_file("test1.pd", "new_name1.pd")
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_complete_processing_workflow(self):
        ...
        with manager.processing_context(total_files=3, description="Test Processing") as ctx:
            ...
            ctx.complete_file("document1.pd", "processed_document1.pd")

            # Process second file with warning
            ctx.start_file("document2.pd")
            ctx.show_warning("OCR quality may be poor")
            ctx.complete_file("document2.pd", "processed_document2.pd")

            # Third file fails
            ctx.start_file("document3.pd")
            ctx.fail_file("document3.pd", "Corrupted file")
    ...
    def test_error_recovery_workflow(self):
        ...
        try:
            with manager.processing_context(total_files=2) as ctx:
                ctx.start_file("good_file.pd")
                ctx.complete_file("good_file.pd", "processed_good.pd")
...
class TestRichUIEnhancements(unittest.TestCase, RichTestCase):
    ...
    def test_target_filename_display_in_progress(self):
        """Test that progress bar shows target filename like: → quarterly_report_2024_financials.pd"""
        ...
        with manager.processing_context(1, "Processing Documents") as ctx:
            ctx.start_file("input_document.pd")
            ctx.set_status("extracting_content")
            ctx.set_status("generating_filename")
            # This should display the arrow and target filename
            ctx.complete_file("input_document.pd", "quarterly_report_2024_financials.pd")
    ...
    def test_celebration_levels_based_on_success_rate(self):
        ...
        with manager.processing_context(2, "Testing Celebrations") as ctx:
            ctx.complete_file("file1.pd", "success1.pd")
            ctx.complete_file("file2.pd", "success2.pd")
        ...
        with manager2.processing_context(3, "Testing Partial Success") as ctx:
            ctx.complete_file("file1.pd", "success1.pd")
            ctx.complete_file("file2.pd", "success2.pd")
            ctx.fail_file("file3.pd", "Processing failed")
    ...
    def test_no_duplicate_completion_messages(self):
        ...
        with manager.processing_context(2, "Test Processing") as ctx:
            ctx.complete_file("file1.pd", "success1.pd")
            ctx.complete_file("file2.pd", "success2.pdf")
    ...
    def test_adaptive_messaging_system(self):
        ...
        for mode, quiet, verbose in test_cases:
            ...
            if quiet:
                ...
                self.assertNotIn("Debug message", content, "Debug should be hidden in {mode}")
                self.assertNotIn("Info message", content, "Info should be hidden in {mode}")
            elif verbose:
                # Verbose mode should show debug messages
                self.assertIn("Debug message", content, "Debug should be shown in {mode}")
                self.assertIn("Info message", content, "Info should be shown in {mode}")
            else:

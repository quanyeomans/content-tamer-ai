...
from utils.rich_progress_display import RichProgressStats
...
class TestCoreContracts(unittest.TestCase):
    ...
    def test_success_rate_calculation_contract(self):
        ...
        for total, succeeded, expected_rate in test_cases:
            ...
            self.assertAlmostEqual(
                ...
                msg=f"Success rate incorrect for {succeeded}/{total}: expected {expected_rate}%, got {calculated_rate}%"
            )
    ...
    def test_counter_increment_contract(self):
        ...
        self.assertLessEqual(
            ...
            f"Processed count {total_processed} exceeds total {stats.total}"
        )
    ...
    def test_no_zero_success_rate_with_successes_contract(self):
        ...
        self.assertEqual(
            ...
            f"Success rate calculation incorrect: expected 100.0%, got {calculated_rate}%"
        )
    ...
    def test_processed_files_list_contract(self):
        ...
        self.assertIsInstance(
            ...
            f"Processed files must be a list, got {type(processed_files)}"
        )
    ...
    def test_data_format_consistency_contract(self):
        ...
        for key, value in completion_data.items():
            self.assertIsInstance(
                value, (int, float, str),
                f"Completion data '{key}' not serializable: {type(value)}"
            )
    ...
    def test_batch_processing_counter_consistency_contract(self):
        ...
        for operation, success_inc, fail_inc in batch_operations:
            ...
            self.assertLessEqual(
                total_processed, stats.total,
                f"Batch processing: processed {total_processed} exceeds total {stats.total}"
            )
            ...
            if total_processed > 0:
                ...
                self.assertIsInstance(
                    ...
                    f"Success rate not numeric during batch processing: {type(actual_rate)}"
                )
    ...
    def test_mixed_results_accuracy_contract(self):
        ...
        self.assertAlmostEqual(
            ...
            msg=f"Mixed results success rate incorrect: expected {expected_success_rate}%, got {actual_success_rate}%"
        )

...
class TestIntegrationContracts(unittest.TestCase, RichTestCase):
    ...
    def setUp(self):
        ...
        self.test_file = os.path.join(self.input_dir, "test_document.pd")
    ...
    def test_file_processing_to_display_contract(self):
        ...
        with display_manager.processing_context(1, "Integration Contract Test") as ctx:
            ...
            ctx.complete_file("test_document.pd", "ai_organized_test_document.pd")
            ...
            self.assertIn(
                "test_document.pd", source_files, "Processed file not recorded in display manager"
            )
    ...
    def test_success_failure_determination_contract(self):
        ...
        with display_manager.processing_context(2, "Success/Failure Contract") as ctx:
            ...
            ctx.start_file("success.pd")
            ctx.complete_file("success.pd", "organized_success.pd")

            # Test failed processing contract
            ctx.start_file("failure.pd")
            ctx.fail_file("failure.pd", "Processing failed")
    ...
    def test_directory_operations_contract(self):
        ...
        for dir_name, dir_path in test_dirs.items():
            self.assertTrue(
                ...
                "Required directory '{dir_name}' does not exist: {dir_path}",
            )
            self.assertTrue(os.path.isdir(dir_path), "Path '{dir_path}' is not a directory")

        # Contract: Test file must be accessible for processing
        self.assertTrue(
            os.path.exists(self.test_file), "Test file does not exist: {self.test_file}"
        )
    ...
    def test_error_handling_integration_contract(self):
        ...
        with display_manager.processing_context(3, "Error Handling Contract") as ctx:
            ...
            ctx.start_file("failure.pd")
            ctx.fail_file("failure.pd", "Simulated processing error")

            # Process another successful file (recovery test)
            ctx.start_file("recovery.pd")
            ctx.complete_file("recovery.pd", "organized_recovery.pd")
    ...
    def test_batch_processing_contract(self):
        ...
        batch_files = [
            ("batch_file_1.pd", "ai_organized_batch_file_1.pd", "success"),
            ("batch_file_2.pd", "", "fail"),
            ("batch_file_3.pd", "ai_organized_batch_file_3.pd", "success"),
            ("batch_file_4.pd", "ai_organized_batch_file_4.pd", "success"),
            ("batch_file_5.pd", "", "fail"),
        ]

        with display_manager.processing_context(
            len(batch_files), "Batch Processing Contract"
        ) as ctx:
            successful_count = 0
            failed_count = 0
            ...
            stats = ctx.display.progress.stats
            self.assertEqual(
                stats.succeeded, successful_count, "Batch processing success count incorrect"
            )
            self.assertEqual(stats.failed, failed_count, "Batch processing failure count incorrect")
            self.assertEqual(
                stats.total, len(batch_files), "Batch processing total count incorrect"
            )

            # Contract: Processed files list must contain all batch items
            processed_files = stats._processed_files
            self.assertEqual(
                len(processed_files), len(batch_files), "Batch processing didn't record all files"
            )
            ...
            self.assertEqual(
                len(failed_files), failed_count, "Failed files count mismatch in batch processing"
            )
    ...
    def test_component_interface_stability_contract(self):
        ...
        for method_name in required_methods:
            self.assertTrue(
                ...
                "RichDisplayManager missing required method: {method_name}",
            )
            self.assertTrue(
                callable(getattr(manager, method_name)),
                "RichDisplayManager.{method_name} is not callable",
            )
        ...
        with manager.processing_context(1, "Interface Test") as ctx:
            ...
            for method_name in context_methods:
                self.assertTrue(
                    hasattr(ctx, method_name),
                    "Processing context missing required method: {method_name}",
                )
                self.assertTrue(
                    callable(getattr(ctx, method_name)),
                    "Processing context.{method_name} is not callable",
                )

            # Contract: Context must have display attribute with stats
            self.assertTrue(hasattr(ctx, "display"), "Processing context missing display attribute")
            self.assertTrue(hasattr(ctx.display, "progress"), "Display missing progress attribute")
            self.assertTrue(
                hasattr(ctx.display.progress, "stats"), "Progress missing stats attribute"
            )
    ...
    def test_data_integrity_across_components_contract(self):
        ...
        with display_manager.processing_context(1, test_data["processing_description"]) as ctx:
            ...
            processed_files = ctx.display.progress.stats._processed_files
            self.assertEqual(
                len(processed_files), 1, "Data integrity: wrong number of processed files"
            )
            ...
            self.assertLess(
                abs(time.time() - timestamp), 60, "Data integrity: timestamp unreasonable"
            )

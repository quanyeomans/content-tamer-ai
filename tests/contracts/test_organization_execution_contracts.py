...
from domains.organization import FolderService
...
class TestOrganizationExecutionContracts(unittest.TestCase, RichTestCase):
    ...
    def test_physical_file_movement_contract(self):
        ...
        for i in range(3):
            ...
            self.assertTrue(
                os.path.exists(target_path), "File test_doc_{i}.pdf not found in target location"
            )

            # Contract: Files must be removed from source locations
            self.assertFalse(
                os.path.exists(source_files[i]),
    ...
    def test_folder_structure_creation_contract(self):
        ...
        organization_plan = {
            ...
            "organization_structure": {
                "folders": {
                    "Documents": {
                        "type": "category",
                        "subfolders": {"2024": {"type": "time"}, "2023": {"type": "time"}},
                    },
                    "Images": {"type": "category"},
                    "Archives": {
                        "type": "category",
                        "subfolders": {"Legal": {"type": "subcategory"}},
                    },
                },
        ...
        for folder in expected_folders:
            ...
            self.assertTrue(os.path.exists(folder_path), "Expected folder not created: {folder}")
    ...
    def test_file_collision_safety_contract(self):
        ...
        self.assertTrue(
            os.path.exists(existing_file), "Original file was lost during collision handling"
        )
        ...
        self.assertTrue(
            os.path.exists(moved_file_path), "Moved file not found at {moved_file_path}"
        )
        self.assertNotEqual(
            moved_file_name, "document.pd", "Collision not handled - filename not changed"
        )
    ...
    def test_partial_failure_graceful_handling_contract(self):
        ...
        for i in range(3):
            ...
            self.assertTrue(
                os.path.exists(target_path), "Valid file {i} was not moved despite partial failures"
            )
    ...
    def test_plan_validation_requirement_contract(self):
        ...
        invalid_plan = {"success": False, "error": "Classification failed"}
        ...
        folder_count = len(
            [
                ...
                if os.path.isdir(os.path.join(self.temp_dir, item)) and not item.startswith(".")
            ]
        )

...
class TestOrganizationExecutionContracts(unittest.TestCase, RichTestCase):
    ...
    def test_physical_file_movement_contract(self):
        ...
        for i in range(3):
            target_path = os.path.join(self.temp_dir, 'Documents', f'test_doc_{i}.pdf')
            self.assertTrue(os.path.exists(target_path), 
                           f"File test_doc_{i}.pdf not found in target location")
            
            # Contract: Files must be removed from source locations
            self.assertFalse(os.path.exists(source_files[i]),
                           f"File test_doc_{i}.pdf still exists in source location")
    ...
    def test_folder_structure_creation_contract(self):
        ...
        for folder in expected_folders:
            ...
            self.assertTrue(os.path.exists(folder_path),
                           f"Expected folder not created: {folder}")
    ...
    def test_file_collision_safety_contract(self):
        ...
        # Create source file
        source_file = os.path.join(self.temp_dir, "document.pdf")
        ...
        self.assertTrue(os.path.exists(moved_file_path),
                       f"Moved file not found at {moved_file_path}")
        self.assertNotEqual(moved_file_name, "document.pdf",
    ...
    def test_partial_failure_graceful_handling_contract(self):
        ...
        for i in range(3):
            target_path = os.path.join(self.temp_dir, 'Documents', f'valid_{i}.pdf')
            self.assertTrue(os.path.exists(target_path),
                           f"Valid file {i} was not moved despite partial failures")
    ...
    def test_atomic_operation_consistency_contract(self):
        ...
        # Create source file
        source_file = os.path.join(self.temp_dir, "atomic_test.pdf")
        with open(source_file, 'w') as f:
        ...
        if result['files_moved'] > 0:
            ...
            target_path = os.path.join(self.temp_dir, 'Documents', 'atomic_test.pdf')
            self.assertTrue(os.path.exists(target_path),
